 ...
 */
function Char(global, container) {
	'use strict';
	...
	var render = function () {
		...
		camera.position.y = (83/(ch/2))*1.5+0.2;
		...
		if(!isPaused) {
			...
			rightarm.rotation.x = runtime.handRadX;
			rightarm.rotation.z = 2.7 + (0.1 * (1.45 - runtime.handRadX));
			rightarm.rotation.y = 0.15;

			// rightarm.rotation.z = 2 * Math.cos(0.6662 * handRad*10 + Math.PI);
			// rightarm.rotation.x = 1 * (Math.cos(0.2812 * handRad*10) - 1);
